...
public:
  ...
  GP operator *(const GP b) const { return GP::power(log() + b.log()); }
  GP operator *=(const GP b) { return *this = *this * b; }
  ssize_t log() const { return logs[v]; }
  static GP power(ssize_t p) { return GP(powers[p]); }

  bool operator ==(const GP other) const { return v == other.v; }
  uint32_t value() const { return v; }

  // Primitive polinomial x8 + x4 + x3 + x2 + 1 (285)
  static const constexpr uint8_t powers[512] = {
  ...
  static constexpr const uint8_t nan = 0;
  static const constexpr uint8_t logs[256] = {
      nan, 0,   1,   25,  2,   50,  26,  198, 3,   223, 51,  238, 27,  104, 199,
      ...
      175};
  static const constexpr uint8_t inverse[256] = {
      nan, 1,   142, 244, 71,  167, 122, 186, 173, 157, 221, 152, 61,  170, 93,
